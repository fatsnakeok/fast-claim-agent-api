...
public class Customer {
    ...
    // 动态计算年龄，避免存储可推导数据，保证数据一致性
    public int getAge() {
        return Period.between(dateOfBirth, LocalDate.now()).getYears();
    }

    // Sentinel 占位对象 — Agent UTILITY 规划器在 LLM 查找客户失败时需要类型化槽位填入 Blackboard，否则会 STUCK
    public static Customer lookupFailed() {
        log.warn("客户查找失败，创建 Sentinel 占位对象");
        Customer c = new Customer();
        c.userId = "__sentinel__";
        return c;
    }

    public static boolean isLookupFailed(Customer c) {
        return "__sentinel__".equals(c.getUserId());
    }
